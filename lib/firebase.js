    ...
    const req = https.request(opts, res => {
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          const hata = `Firestore ${res.statusCode}: ${body}`;
          console.error('Firestore istek:', method, opts.path);
          console.error('Firestore body:', data ? data.substring(0, 500) : '(bos)');
...
async function setDocument(docId, data) {
  ...
  return firestoreRequest(token, 'PATCH', `${COLLECTION}/${docId}`, { fields: toFields(data) });
}
